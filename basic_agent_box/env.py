...
class SchedEnv(gym.Env):
    def __init__(self, env_config):
        ...
        self.observation_space = Dict({
                ...
                "ready_tasks": Box(low= 0, high=max(self.M, self.N), shape=(self.N, 6)), # El tipo de tarea (tiempo que tarda con cada una de los 5 tamaños e instancia) de las hasta N tareas libres, y la cantidad de tareas que hay de ese tipo (componente 6)
                "slices_t": Box(low= 0, high=self.M, shape=(7,)) # Cuanto le queda (de 0 a M) a cada slice
            })
    ...
    def _get_numpy_obs_state(self):
        return OrderedDict([('action_mask', np.array(self.obs["action_mask"], dtype=np.int8)),
                            ('partition', np.int64(self.obs["partition"] - 1)), # El objeto lleva las particiones de 1 a 19, pero el agente de 0 a 18 
                            ('ready_tasks', np.array(self.obs["ready_tasks"], dtype=np.float32)),
                            ('slices_t', np.array(self.obs["slices_t"], dtype=np.float32))])
