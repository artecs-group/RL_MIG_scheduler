...
class SchedEnv(gym.Env):
    ...
    def reset(self, seed = None, options = None):
        ...
        self.tasks = get_ready_tasks(self.type_tasks, 50)
        tasks, self.reconfig_time_scaled = time_discretization(self.tasks, self.M, self.reconfig_time)
        ready_tasks = tasks[:self.N]
        self.pending_tasks = tasks[self.N:]
        ...
        self.num_type_task = list(range(self.N))
    ...
    def step(self, action):
        ...
        # Reconfigurar
            ...
            elapse_time = max([slices_t[slice_pos] for slice_pos in range(7) if partition_map[action]["instances"][slice_pos] != partition_map[current_partition]["instances"][slice_pos]])
            # Dejamos apsar el tiempo para todos los slices
        ...
        # Asignar tarea
            ...
            # Si hay tareas pendientes, añadimos la primera de ellas a ready
            if len(self.pending_tasks) > 0:
                new_task = self.pending_tasks.pop(0)
                new_task_c = [time_c for time_c, _ in new_task]
                new_task_d = [time_d for _, time_d in new_task]
                ready_tasks = insert_canonical(self.M, ready_tasks, new_task_d)
                num_new_task = type_num_task(self.M, new_task_d)
                if num_new_task in self.dic_cont_times:
                    self.dic_cont_times[num_new_task].append(new_task_c)
                else:
                    self.dic_cont_times[num_new_task] = [new_task_c]
                ready_tasks += [[0] * 6] * (self.N - len(ready_tasks))

            else: # Si no hay tareas pendientes
                # Añadimos un tipo de tarea vacío al final para mantener la dimensionalidad
                if len(ready_tasks) < self.N:
            ...
            self.obs["ready_tasks"] = ready_tasks
            reward = 0
...
if __name__ == "__main__":
    env_example = SchedEnv({"N": 5, "M": 7}, type_tasks="good_scaling")
