...
def _num_task_to_times(numbered_tasks):
    dic_cont, dic_discrete, dic_names = {}, {}, {}
    for num, name, task in numbered_tasks:
        if num not in dic_cont:
            ...
            dic_names[num].append(name)
    return dic_cont, dic_discrete, dic_names

def canonical_sort_tasks(M, tasks):
    # Pongo el número de tipo de tarea a cada una
    numbered_tasks = [(type_num_task(M, [time_d for _, time_d in task]), name, task) for name, task in tasks]
    dic_cont, dic_discrete, dic_names = _num_task_to_times(numbered_tasks)
    print(dic_cont, dic_discrete, dic_names)
    ...
    return canonical_tasks, dic_cont, dic_names
...
def time_discretization(ready_tasks, M, reconfig_time):
    times = [task_times[1:] for task_times in ready_tasks]
    max_time = max(max(task) for task in times)
    time_step = max_time / M
    reconfig_time_scaled = reconfig_time / time_step
    scaled_times = [(task[0], [(time, round(time / time_step) + 1) for time in task[1:]]) for task in ready_tasks]
    return scaled_times, reconfig_time_scaled
